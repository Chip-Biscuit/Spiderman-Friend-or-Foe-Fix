...
// Fix Enhancers/ Chips Spiderman Friend or Foe Patches Added
...
bool GetResolutionSettings(int& resolutionX, int& resolutionY)
{
    HKEY hKey;
    DWORD dwResolutionX = 0;
    DWORD dwResolutionY = 0;
    DWORD dwSize = sizeof(DWORD);

    if (RegOpenKeyEx(HKEY_CURRENT_USER, _T("SOFTWARE\\Activision\\Spider-Man FoF\\Settings"), 0, KEY_READ, &hKey) == ERROR_SUCCESS)
    {
        if (RegQueryValueEx(hKey, _T("ResolutionX"), NULL, NULL, (LPBYTE)&dwResolutionX, &dwSize) == ERROR_SUCCESS &&
            RegQueryValueEx(hKey, _T("ResolutionY"), NULL, NULL, (LPBYTE)&dwResolutionY, &dwSize) == ERROR_SUCCESS)
        {
            resolutionX = static_cast<int>(dwResolutionX);
            resolutionY = static_cast<int>(dwResolutionY);
            RegCloseKey(hKey);
            return true;
        }
        RegCloseKey(hKey);
...
void SetResolutionSettings(int resolutionX, int resolutionY)
{
    HKEY hKey;
    DWORD dwResolutionX = static_cast<DWORD>(resolutionX);
    DWORD dwResolutionY = static_cast<DWORD>(resolutionY);

    if (RegOpenKeyEx(HKEY_CURRENT_USER, _T("SOFTWARE\\Activision\\Spider-Man FoF\\Settings"), 0, KEY_WRITE, &hKey) == ERROR_SUCCESS)
    {
        RegSetValueEx(hKey, _T("ResolutionX"), 0, REG_DWORD, (const BYTE*)&dwResolutionX, sizeof(DWORD));
        RegSetValueEx(hKey, _T("ResolutionY"), 0, REG_DWORD, (const BYTE*)&dwResolutionY, sizeof(DWORD));
        RegCloseKey(hKey);
    }
}

// Function to read the resolution settings from an INI file
bool ReadIniSettings(const TCHAR* iniFilePath, int& resolutionX, int& resolutionY)
{
    TCHAR resultX[10], resultY[10];
    GetPrivateProfileString(_T("Resolution"), _T("ResolutionX"), _T("0"), resultX, 10, iniFilePath);
    GetPrivateProfileString(_T("Resolution"), _T("ResolutionY"), _T("0"), resultY, 10, iniFilePath);
    resolutionX = _ttoi(resultX);
    resolutionY = _ttoi(resultY);
...
void ToggleResolutionSettings(HMODULE hModule)
{
    ...
    if (ReadIniSettings(dllPath, resolutionX, resolutionY))
    {
        SetResolutionSettings(resolutionX, resolutionY);
    }
}
...
void PerformHexEdits2() {
    ...
    float predefinedFloats[] = { 0.021f, 0.025f, 0.029f };
